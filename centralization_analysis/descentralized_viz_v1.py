...
html.Div([
    dcc.Graph(
        id='vpm_treemap',
        figure=build_treemap(df_val_per_month.loc[df_val_per_month['Month'].max()], 0, 0, 200, 100),
        config={
...
html.Div([
        dcc.Graph(
            id='cpm_treemap',
            figure=build_treemap(df_ct_per_month.loc[df_ct_per_month['Month'].max()], 0, 0, 200, 100),
            config={
                'displayModeBar': False
            }
        )
...
@app.callback(
     ...
     [dash.dependencies.Input('date_slider', 'date')])
def update_vpm_treemap(date):
    return build_treemap(date,0,0,500,500)
